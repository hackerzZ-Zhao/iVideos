...
class ExternalVideo(View):
    ...
    def post(self, request):
        ...
        if not all([name, image, video_type, from_to, nationality, info]):
            return redirect('{}?error={}'.format(reverse_path, 'Missing Required Information!'
                                                               ''))

        result = check_and_get_video_type(
            VideoType, video_type, 'Invalid Video Type!')
        if result.get('code') != 0:
            return redirect('{}?error={}'.format(reverse_path, result['msg']))

        result = check_and_get_video_type(
            FromType, from_to, 'Invalid Resource!')
        if result.get('code') != 0:
            return redirect('{}?error={}'.format(reverse_path, result['msg']))

        result = check_and_get_video_type(
            NationalityType, nationality, 'Invalid Nationality!')
        if result.get('code') != 0:
            return redirect('{}?error={}'.format(reverse_path, result['msg']))
        ...
        if not video_id:
            try:
                ...
                return redirect('{}?error={}'.format(reverse_path, 'Fail'))
        else:
            try:
...
class VideoSubView(View):
    ...
    def post(self, request, video_id):
        ...
        if not all([url, number]):
            return redirect('{}?error={}'.format(url_format, 'Missing Required Information!'))
        ...
        if not videosub_id:
            try:
                ...
                return redirect('{}?error={}'.format(url_format, 'Fail!'))
        else:
...
class VideoStarView(View):
    ...
    def post(self, request):
        ...
        if not all([name, identity, video_id]):
            return redirect('{}?error={}'.format(path_format, 'Missing Required Information!'))

        result = check_and_get_video_type(
            IdentityType, identity, 'Invalid Identity!')
        ...
        try:
            ...
            return redirect('{}?error={}'.format(path_format, 'Fail!'))
